...
def set_params(command_line_args, create_files=True):
    ...
    from utilities.algorithm.initialise_run import (
        initialise_run_params,
        set_param_imports,
    )
    from utilities.fitness.math_functions import return_one_percent
    from utilities.stats import clean_stats, trackers
    ...
    if params["LOAD_STATE"]:
        ...
        if (
            params["MUTATION_PROBABILITY"] is not None
            and params["MUTATION_EVENTS"] != 1
        ):
            s = (
        ...
        if params["TARGET_SEED_FOLDER"]:
            ...
            params["SEED_INDIVIDUALS"] = load_population(params["TARGET_SEED_FOLDER"])
