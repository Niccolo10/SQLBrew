...
class Grammar(object):
        ...
        with open(file_name, "r") as bnf:
            ...
            for rule in finditer(self.ruleregex, content, DOTALL):
                ...
                if self.start_rule is None:
                    ...
                    self.start_rule = {"symbol": rule.group("rulename"), "type": "NT"}
                ...
                for p in finditer(
                    ...
                    if terminalparts is not None:
                        ...
                        if terminalparts not in self.terminals:
                            ...
                            rule.group("rulename") not in self.terminals[terminalparts]
                        ):
                            self.terminals[terminalparts].append(rule.group("rulename"))
                    tmp_productions.append(
                        {"choice": tmp_production, "recursive": False, "NT_kids": False}
                    )

                if rule.group("rulename") not in self.rules:
                    ...
                    if len(tmp_productions) == 1:
                        # Unit productions.
                        print(
                            "Warning: Grammar contains unit production "
                            "for production rule",
                            rule.group("rulename"),
                        )
    ...
    def check_depths(self):
        ...
        for rule in sorted(self.rules.keys()):
            ...
            self.non_terminals[rule]["b_factor"] = self.rules[rule]["no_choices"]
    ...
    def set_arity(self):
        ...
        self.min_path = self.non_terminals[self.start_rule["symbol"]]["min_steps"]
        ...
        for rule in self.rules:
            for prod in self.rules[rule]["choices"]:
                for sym in [i for i in prod["choice"] if i["type"] == "NT"]:
                    sym["recursive"] = self.non_terminals[sym["symbol"]]["recursive"]
                    if sym["recursive"]:
                        prod["recursive"] = True
    ...
    def find_concatenation_NTs(self):
        ...
        for rule in sorted(self.rules.keys()):
            ...
            concat = [
                choice for choice in self.rules[rule]["choices"] if choice["NT_kids"]
            ]

            if concat:
                # We can reduce_trees NTs.
                for choice in concat:
                    symbols = [[sym["symbol"], sym["type"]] for sym in choice["choice"]]

                    NTs = [
                        sym["symbol"] for sym in choice["choice"] if sym["type"] == "NT"
                    ]
