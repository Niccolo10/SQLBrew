    ...
    if no_seeds > 0:
        ...
        for ind in params["SEED_INDIVIDUALS"]:
            if not isinstance(ind, individual.Individual):
                ...
                individuals.extend([ind.deep_copy() for _ in range(num_per_seed)])
        ...
        if genotype:
            ...
            if phenotype and ind.phenotype != phenotype:
                s = (
                    ...
                    "grammar to ensure all is correct: " + params["GRAMMAR_FILE"]
                )
                raise Exception(s)
...
def LTGE_initialisation(size):
    ...
    for _ in range(size):
        # Random genotype
        g, ph = latent_tree_random_ind(params["BNF_GRAMMAR"], params["MAX_TREE_DEPTH"])
